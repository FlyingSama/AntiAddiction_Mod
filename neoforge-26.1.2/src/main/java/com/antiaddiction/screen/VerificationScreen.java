...
public class VerificationScreen extends Screen {
    ...
    public void extractBackground(GuiGraphicsExtractor gfx, int mouseX, int mouseY, float partialTick) {
        ...
        String label1 = "姓　　名：";
        int fieldX = cx - FIELD_WIDTH / 2;
        gfx.text(this.font, Component.literal(label1), fieldX, cy - 22, 0xFFFFFFFF);
        String label2 = "身份证号：";
        gfx.text(this.font, Component.literal(label2), fieldX, cy + 18, 0xFFFFFFFF);
    }
    ...
    private void drawScaledBoldCenteredString(GuiGraphicsExtractor gfx, Component text, int cx, int y, float scale, int color) {
        int scaledX = Math.round(cx / scale) - this.font.width(text) / 2;
        int scaledY = Math.round(y / scale);
        gfx.pose().pushMatrix();
        gfx.pose().scale(scale, scale);
        gfx.text(this.font, text, scaledX, scaledY, color, true);
        gfx.pose().popMatrix();
    }

    private void drawPopup(GuiGraphicsExtractor gfx, int cx, int cy, String msg, int bg, int border, int textColor) {
        ...
        gfx.centeredText(this.font, Component.literal(msg), cx, py + (ph - this.font.lineHeight) / 2, textColor);
    }
